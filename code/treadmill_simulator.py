...
class TreadmillSimulator:
    def __init__(
        ...
        self.current_speed = self._validate_initial_speed(
            initial_speed
            )
    ...
    def _validate_initial_speed(
            self,
            speed
            ):
        self._check_speed_type(
            speed
            )
    ...
    def _check_speed_type(
        ...
        if not isinstance(
            ...
            raise TypeError(
                "Speed must be an integer or a float."
                )

    def _check_speed_value(
            self,
            speed
            ):
        if speed < 0:
    ...
    def _run(self):
        ...
        while self.running:
            self._update_distance_covered(
                last_time
                )
            last_time = self._get_current_time()
            self._sleep_one_second()

    def _update_distance_covered(
            self,
            last_time
            ):
        with self.lock:
            ...
            self.distance_covered += self._calculate_distance(
                elapsed_time
                )
    ...
    def _calculate_elapsed_time_since_start(
            self
            ):
    ...
    def get_current_speed(
        ...
        return self._get_value_with_lock(
            self.current_speed
            )
