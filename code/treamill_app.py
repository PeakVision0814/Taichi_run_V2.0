...
class TreadmillApp:
    ...
    def _update_target_input(
            self,
            target_type,
            value
            ):
        self.target_type_combo.set(
            target_type
            )
        self._enable_entry(
            self.target_entry
            )
        self._clear_and_set_entry(
            self.target_entry,
            value
            )

    def _clear_and_set_entry(
            self,
            entry,
            value
            ):
    ...
    def update_target_entry(
            self,
            event
            ):
        target_type = self.target_type_combo.get()
        self._update_target_label_and_entry(
            target_type
            )

    def _update_target_label_and_entry(
            self,
            target_type
            ):
        if target_type == "距离（米）":
    ...
    def _destroy_goal_reached_dialog(
            self
            ):
    ...
    def on_goal_reached_dialog_continue(
            self
            ):
        self.goal_reached_dialog.destroy(
    ...
    def on_goal_reached_dialog_stop(
            self
            ):
        self.goal_reached_dialog.destroy()
        self.stop_workout()
    ...
    def _update_distance_label(
            self, 
            distance
            ):
        self.distance_label.config(
            text=distance.split(": ")[1]
            )
    ...
    def _show_warning(
        ...
        warning_window = self._create_warning_window(
            "警告",
            "200x100"
            )
        ...
        self._create_warning_button(
            warning_window,
            "确定"
            )

    def _create_warning_window(
            self,
            title,
            geometry
            ):
